...
def plot_2D_data_result(X_data: pd.DataFrame, df_result: pd.DataFrame):
...
def get_dataset(name):
    if name == 'Original':
        ...
        # ,'thalachh','oldpeak','thall']
        vars_to_use = ['age', 'trtbps', 'chol', 'thalachh']
    elif name == 'Expanded':
        file = 'Expanded.xlsx'
        vars_to_use = ['x1', 'x2', 'x3', 'x4', 'x5']
...
def calculate_membership(X_data: pd.DataFrame, centroides: list):
    ...
    for i in range(len(X_data)):
        ...
        for j in range(len(centroides)):
            dist_cent = normas.norma_euclidea(
                X_data.loc[i].tolist(), centroides[j])
            if dist_cent <= dist:
                dist = dist_cent
