...
def density(X_data: pd.DataFrame, centroide: list, highest_val: float, rb: float) -> list:
    ...
    for i in range(len(X_data)):
        ...
        val = X_data.loc[i, 'Value'] - highest_val * \
            math.exp(-(normas.norma_euclidea(v1, v2)**2)/((rb/2)**2))
        values.append(val)
    return values
...
def execute(n_iterations, ra, rb, file, vars_to_use, numpy_or_pandas):
    ...
    for i in range(n_iterations):
        if i == 0:
            ...
            centroides.append(
                X_data.loc[centroid_idx, X_data.columns != 'Value'].tolist())
            X_data = calc_mnt_grid(
                X_data, centroides[-1], highest_val, ra, rb, i)
            results.append(X_data)
            if len(centroides) >= 2 and centroides[-2] == centroides[-1]:
                break
    centroides = centroides[:-1]
    df_result = commons.calculate_membership(
        X_data.loc[:, X_data.columns != 'Value'], centroides)
...
if __name__ == '__main__':
    file, vars_to_use = commons.get_dataset('Compressed')
    numpy_or_pandas = 'pandas'
    n_iterations = 100
    ra = 2.45
    rb = 3.0
    df_result, centroides = execute(
