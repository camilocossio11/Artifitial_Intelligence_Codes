...
def get_centroids(U, X_data, n_samples, k, m):
    centroids = []
    for i in range(k):
...
def execute(file, vars_to_use, numpy_or_pandas, k, m, max_iter=1000):
    ...
    for i in range(max_iter):
        if len(costs) >= 2 and costs[-2] - costs[-1] < 0.0001:
            break
        # Get centroids
        centroids = get_centroids(U, X_data, n_samples, k, m)
        # Get distance matrix
...
if __name__ == '__main__':
    ...
    k = 2
    m = 2
    costs, centroids, U, X_data = execute(
        file, vars_to_use, numpy_or_pandas, k, m)
    silhouette_avg = commons.silhouette(
        X_data[vars_to_use], X_data['Label'].tolist())
